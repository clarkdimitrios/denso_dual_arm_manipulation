...
def generate_launch_description():
    ...
    # ======================================================
    # STEP 3: Dual-URDF + MoveIt
    # ======================================================
    # For SIMULATION: launch Gazebo + spawn left/right arm controllers
    dual_moveit_sim = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(denso_bringup_launch),
        launch_arguments={
            'model': 'vm60b1',
            'ip_address': '192.168.17.50',  # dummy / unused in sim
            ...
            'moveit_controllers_file': 'dual_moveit_controllers_sim.yaml',
            'robot_controller': 'left_arm_controller right_arm_controller',
            'sim': 'true',
            'launch_rviz': 'true',
            'launch_moveit': 'true',
            'launch_hw': 'true',
    ...
    # For REAL HARDWARE: no Gazebo, no controller spawning (handled by individual arm launches)
    dual_moveit_hw = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(denso_bringup_launch),
        launch_arguments={
            'model': 'vm60b1',
            'ip_address': '192.168.17.50',  # dummy / unused
            'description_file': 'dual_denso_robot.urdf.xacro',
            'moveit_config_file': 'dual_denso_robot.srdf.xacro',
            'controllers_file': 'dual_denso_robot_controllers.yaml',
    ...
    dual_moveit_sim_timed = TimerAction(
        period=t_mux_to_right + t3,
        actions=[dual_moveit_sim],
    )

    dual_moveit_hw_timed = TimerAction(
        period=t_mux_to_right + t3,
        actions=[dual_moveit_hw],
    )
    ...
    return LaunchDescription(
        ...
        + [
            ...
            # Step 1–2 (hardware only)
            right_hw,
            left_hw,

            # Step 3 (sim or hw)
            dual_moveit_sim_timed,
            dual_moveit_hw_timed,
